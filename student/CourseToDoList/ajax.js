var response;
function createTodo(course_ID){
    $.ajax('./createTodo.php', {
        type: 'POST',
        dataType: "json",
        data: { course_ID: course_ID, newTodo: $("#newTodoInput").val(), }, // data to submit
...
function loadTodos(course_ID){
    $.ajax('./loadTodo.php', {
        type: 'POST',
        dataType: "json",
        data: { course_ID: course_ID, }, // data to submit
        success: function (data, status, xhr) {
            response = data;
            if(data.status){
                $('#TodosBody tr').remove();
                $('#TodosBody').append(`<tr class="d-flex" id="newTodoRow">
                    ...
                    <td class="col-3"><button type="button" class="btn btn-primary" id="createTodoBtn" onclick='createTodo(course_ID);'>Create Todo</button></td>
                </tr>`);
