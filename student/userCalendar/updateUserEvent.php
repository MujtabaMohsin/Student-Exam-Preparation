<?php
    ...
    if($checkParameters) {
        ...
        $query = "UPDATE `user_events` SET `title` = ?, `description` = ?, `start` = ?, `end` = ?, `allDay` = ? WHERE `event_ID` = ?";
        ...
        if($stmt->execute()) {
            ...
            $query = "UPDATE `reminders` SET `start_time` = ?, `end_time` = ? WHERE `event_ID` = ? AND `event_type` = 'user_events'";
            $stmt = $conn->prepare($query);
