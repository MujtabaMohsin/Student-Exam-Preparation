<?php 
    ...
    if(isset($_POST["action"])){
        $query = "SELECT * FROM notes WHERE course_ID = $courseID AND category = 1";
            ...
            if($chapter != $row["chapter"]){
                ...
                echo' <div class="card" style="width: 985.981px;">
                        <div class="card-header active" role="tab">
                            <h5 class="mb-0"><a data-toggle="collapse" aria-expanded="false" aria-controls="accordion-1 .item-'.$acc1.'" href="#accordion-1 .item-'.$acc1.'">Chapter '.$row["chapter"].'</a></h5>
                        </div>
    ...
    echo'
                ...
                <button class="btn btn-danger" style="font-size:14px;float:right; margin-left:1%;margin-right:1%" onclick="window.location=\'delete.php?id='.$row["note_ID"].'\'">Delete</button>
               ';
               if($row["private"] == 1)
                echo '
